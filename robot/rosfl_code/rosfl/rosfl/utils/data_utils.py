...
def read_data(dataset, share_dir, idx, is_train=True):
    if is_train:
        data_dir = os.path.join(share_dir, 'datas/dataset', dataset, 'train/')
    else:
        data_dir = os.path.join(share_dir, 'datas/dataset', dataset, 'test/')
...
def read_client_data(dataset, share_dir, idx, is_train=True, few_shot=0):
    data = read_data(dataset, share_dir, idx, is_train)
    if "News" in dataset:
